 ...
 */
async function analyseResults(features, stats, projectName) {
...
function buildResultsSummary(features, stats, projectName) {
    const lines = [];
    ...
    for (const feature of features) {
        ...
        for (const scenario of feature.scenarios) {
            ...
            if (scenario.status === 'failed') {
                const failedStep = scenario.steps.find(s => s.status === 'FAILED' || s.status === 'failed');
...
// ─── Build Claude prompt ──────────────────────────────────────────────────────
function buildPrompt(resultsSummary, projectName) {
