...
export function generateHTML({ projectName, clientName, environment, features, stats, analysis, generatedAt, logoBase64, logoMimeType }: HtmlOptions): string {
...
function renderAnalysis(analysis) {
  ...
  const riskItems = (analysis.riskFlags || []).map(flag =>
    `<li>${escHtml(flag)}</li>`
  ).join('');
...
function renderScenario(scenario, fi, si) {
  ...
  const tags = (scenario.tags || []).map(t =>
    ...
    <table class="steps-table">
      ...
      <tbody>
        ...
        <tr class="step-${step.status?.toLowerCase()}">
          ...
          <td><span class="pill ${
            step.status === 'PASSED'  ? 'pill-done'     :
            step.status === 'FAILED'  ? 'pill-blocked'  :
            step.status === 'SKIPPED' ? 'pill-deferred' : 'pill-progress'
          }">${escHtml(step.status || '?')}</span></td>
